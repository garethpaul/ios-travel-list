...
The baseline runs `scripts/check-baseline.py`, parses plist/storyboard/asset metadata, checks image resources and Xcode wiring, verifies item trimming and side-effect-free cell rendering, and guards against logging, network, upload, analytics, or persistence behavior.
...
- Cell rendering should remain side-effect free; avoid reloading the table from inside `cellForRowAtIndexPath`.
