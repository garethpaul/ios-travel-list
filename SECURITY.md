...
- `make check` runs a static baseline that guards plist/storyboard metadata, Xcode project wiring, source inventory, the shared name normalizer, normalizer tests, cell index checks, removal index checks, duplicate item checks, navigation logo title view ownership, cell rendering, stale cell reset handling, and logging/network/persistence regressions when Xcode is unavailable.
