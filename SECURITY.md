...
The supported security scope for `ios-travel-list` is the current default branch, `master`. Older commits, tags, branches, forks, demos, and generated artifacts are not actively supported unless the repository explicitly marks them as maintained.

Project summary: Travel List so you don't forget things.

## Reporting a Vulnerability

Please report suspected vulnerabilities through GitHub's private vulnerability reporting or by opening a draft GitHub Security Advisory for `garethpaul/ios-travel-list` when that option is available. If GitHub does not show a private reporting option for this repository, contact the repository owner through GitHub and avoid posting exploit details publicly until the issue can be assessed.
...
- `make check` runs a static baseline that guards plist/storyboard metadata, Xcode project wiring, source inventory, the shared name normalizer, embedded control-character rejection, collection add-boundary normalization, normalizer tests, cell index checks, removal index checks, duplicate item checks, navigation logo title view ownership, cell rendering, stale cell reset handling, and logging/network/persistence regressions when Xcode is unavailable.
...
- The pinned macOS GitHub Actions workflow is read-only and compiles the unsigned app and XCTest target without
